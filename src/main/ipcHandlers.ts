...
export function setupIpcHandlers(
  ...
  ipcMain.handle('start-audio-capture', async (_event, config: AudioCaptureConfig) => {
    try {
                ...
                if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
                  try {
                    ...
                    if (parsed.token) {
                      ...
                      reject(new Error('Backend response missing "token" property'));
                    }
                  } catch (e: any) {
                    reject(new Error(`Failed to parse backend JSON response: ${e.message}`));
                  }
                } else {
                  reject(new Error(`Backend HTTP error status ${res.statusCode}: ${data}`));
                }
              });
            }
          );
