...
export class AudioCaptureEngine {
  ...
  public async startCapture(
    ...
    if (config.mode === 'process' && config.targetPid && config.targetPid > 0) {
      const candidatePids = findCandidatePids(config.targetPid, config.targetProcessName);
      console.log(`[AudioEngine] 🎯 Candidate PIDs for isolated process audio:`, candidatePids);

      let lastError: Error | null = null;
      for (const targetPid of candidatePids) {
        try {
          console.log(`[AudioEngine] 🔊 Activating WASAPI Process Loopback — PID: ${targetPid} (${config.targetProcessName}) + child processes`);
          const processSuccess = await this.tryProcessLoopback(targetPid, window);
          if (processSuccess) {
            this.currentStatus = {
              active: true,
              mode: 'process',
          ...
          lastError = err;
          console.warn(`[AudioEngine] ⚠️ Process loopback init failed for PID ${targetPid}:`, err.message);
        }
      }

      console.warn('[AudioEngine] ⚠️ WASAPI Process Loopback could not bind to candidate PIDs. Falling back to system loopback...');
    }
  ...
  private tryProcessLoopback(pid: number, window: BrowserWindow): Promise<boolean> {
    return new Promise((resolve, reject) => {
      try {
        ...
        console.log(`[AudioEngine] ✅ WASAPI Process Loopback active for PID ${pid}`);
        resolve(true);
      } catch (err) {
        reject(err);
      }
