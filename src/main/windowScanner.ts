...
import { WindowSource } from '../types/capture';
...
export async function getProcessMap(): Promise<Map<number, string>> {
  ...
  try {
    const { stdout } = await execAsync('tasklist /FO CSV /NH', { encoding: 'utf8', timeout: 3000 });
    ...
    console.warn('[WindowScanner] Could not execute tasklist for process names:', err);
  }
  return cachedProcessMap;
}
 ...
 */
export function findCandidatePids(primaryPid: number, processName?: string | null): number[] {
  ...
  const gameAliases: Record<string, string[]> = {
    ...
    'riotclientservices.exe': ['leagueclient.exe', 'league of legends.exe', 'valorant-win64-shipping.exe'],
  };
  ...
  for (const [pid, name] of map.entries()) {
    ...
    if (pid !== primaryPid && (lowerName === lowerTarget || aliases.includes(lowerName))) {
      candidates.push(pid);
    }
  }
