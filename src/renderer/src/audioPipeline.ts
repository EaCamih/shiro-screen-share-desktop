export class AudioPipeline {
  ...
  private readonly SCHEDULE_AHEAD_SEC = 0.005; // 5ms ultra-low latency buffer
  ...
  public initialize(): MediaStreamTrack | null {
    ...
    try {
      ...
      if (window.api && window.api.onProcessAudioData) {
        this.pcmUnsubscribe = window.api.onProcessAudioData((arrayBuffer: ArrayBuffer) => {
      ...
      if (audioTracks.length > 0) {
        console.log('[AudioPipeline] ✅ Low-latency AudioTrack initialized (48kHz, AnalyserNode active)');
  ...
  private processPcmChunk(buffer: ArrayBuffer): void {
    if (!this.audioContext || !this.destinationNode || this.audioContext.state === 'closed') return;
