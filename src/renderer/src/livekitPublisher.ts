...
import { StreamQualityOptions } from '../../types/capture';
...
export class LiveKitPublisher {
  ...
  public async connectAndPublish(options: LiveKitPublishOptions): Promise<Room> {
    this.disconnect();
    ...
    if (options.videoTrack) {
      console.log(`[LiveKit] Publishing screen share video track (Bitrate: ${maxBitrateBps / 1000}Kbps, FPS: ${maxFramerate})...`);
    ...
    if (options.audioTrack) {
      console.log('[LiveKit] Publishing isolated process audio track with zero DTX latency...');
  ...
  public async replaceVideoTrack(
    ...
    const videoPub = Array.from(this.room.localParticipant.videoTrackPublications.values()).find(
      (pub) => pub.source === Track.Source.ScreenShare
    );

    if (videoPub && videoPub.track) {
      console.log('[LiveKit] ⚡ Replacing screen share video track in real-time...');
      await (videoPub.track as any).replaceTrack(newTrack);
      await this.updateEncodingParameters(qualityOptions);
      console.log('[LiveKit] ✅ Video track replaced in real-time successfully!');
    }
  }
   ...
   */
  public async updateEncodingParameters(qualityOptions: StreamQualityOptions): Promise<void> {
    if (!this.room || !this.isConnected) return;
      ...
      try {
        ...
        if (sender && typeof sender.getParameters === 'function') {
          ...
          if (params && params.encodings) {
            ...
            params.degradationPreference = qualityOptions.degradationPreference || 'maintain-resolution';
            await sender.setParameters(params);
            console.log(`[LiveKit] ⚡ Real-time parameters updated: ${qualityOptions.bitrateKbps}Kbps, ${maxFramerate}FPS, ${qualityOptions.degradationPreference}`);
          }
        }
      } catch (err) {
        console.warn('[LiveKit] Warning updating RTCRtpSender parameters:', err);
      }
    }
  }
}
