...
import { setupWindowControls, AudioVisualizer, setStreamStatus } from './uiComponents';
import { WindowSource, AudioCaptureMode, StreamQualityOptions } from '../../types/capture';
import { DeepLinkParams } from '../../types/ipc';
...
class ShiroApp {
  ...
  private applyTheme(theme: string): void {
    ...
    const btnToggle = document.getElementById('btn-theme-toggle');
    if (btnToggle) {
      btnToggle.innerHTML = theme === 'dark' ? '<i data-lucide="sun"></i>' : '<i data-lucide="moon"></i>';
      btnToggle.title = theme === 'dark' ? 'Alternar para Tema Claro' : 'Alternar para Tema Escuro';
      this.refreshIcons();
    }
  }
  ...
  private setupSourcePicker(): void {
    ...
    if (gridContainer) {
      this.sourcePicker = new SourcePicker(gridContainer, (selectedSource: WindowSource) => {
  ...
  private async refreshSources(): Promise<void> {
    if (window.api && window.api.getAvailableSources) {
  ...
  private async onSourceSelected(source: WindowSource): Promise<void> {
    ...
    try {
      ...
      const videoElem = document.getElementById('preview-video') as HTMLVideoElement;
      ...
      if (this.livekitPublisher.getIsConnected() && this.currentVideoTrack) {
        console.log('[App] ⚡ Live stream active! Updating video track & settings in real-time...');
        await this.livekitPublisher.replaceVideoTrack(this.currentVideoTrack, quality);
  ...
  private checkCanStartStream(): void {
    const btnStart = document.getElementById('btn-start-stream') as HTMLButtonElement;
    const selectedSource = this.sourcePicker?.getSelectedSource();
  ...
  private getSelectedAudioMode(): AudioCaptureMode {
    const selectedRadio = document.querySelector('input[name="audioMode"]:checked') as HTMLInputElement;
  ...
  private async startStreaming(): Promise<void> {
    ...
    const identityInput = document.getElementById('input-identity') as HTMLInputElement;

    const roomName = roomInput?.value.trim() || 'sala-shiro-default';
    const rawIdentity = identityInput?.value.trim() || `user-${Math.floor(Math.random() * 1000)}`;

    // Discord Activity expects identity ending with '-capture' to bind stream to participant
    const livekitIdentity = rawIdentity.endsWith('-capture') ? rawIdentity : `${rawIdentity}-capture`;

    const audioMode = this.getSelectedAudioMode();
    const qualityOptions = this.getQualityOptions();

    console.log(`[App] 🚀 Starting Stream — Room: ${roomName}, Identity: ${livekitIdentity}, AudioMode: ${audioMode}`);
    ...
    if (audioMode !== 'disabled') {
      ...
      if (analyser) {
        ...
        if (vuStatus) { vuStatus.innerText = 'Recebendo áudio nativo'; vuStatus.style.color = '#10b981'; }
      }
    }
    ...
    try {
      ...
      const livekitWsUrl = (typeof process !== 'undefined' && process.env?.LIVEKIT_URL)
        ? process.env.LIVEKIT_URL
        : defaultLivekit;

      console.log(`[App] Fetching LiveKit Token from ${backendUrl} for ${livekitIdentity}...`);
      const token = await window.api.fetchLiveKitToken(backendUrl, roomName, livekitIdentity);
  ...
  private stopStreaming(): void {
    ...
    const vuStatus = document.getElementById('vu-status-text');
    if (vuStatus) { vuStatus.innerText = 'Aguardando som...'; vuStatus.style.color = ''; }
  ...
  private setupDeepLinkListener(): void {
    if (window.api && window.api.onDeepLinkReceived) {
      window.api.onDeepLinkReceived((params: DeepLinkParams) => {
        ...
        if (params.identity || params.userName || params.userId) {
          const identityInput = document.getElementById('input-identity') as HTMLInputElement;
          const idVal = params.userId || params.identity || params.userName || '';
